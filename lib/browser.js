...
var Browser = function(callback) {
  ...
  this.parser.on("response", function(error, _cbid, response) {
    this.callbacks[_cbid].call(this, error, response);
  }.bind(this));
...
Browser.prototype.renderTile = function(width, height, tile_width, tile_height, callback) {
  this.command("RenderTile", width, height, tile_width, tile_height, callback);
};
...
Browser.prototype.setHtml = function(html, baseUrl, callback) {
  if (callback) {
    this.command("SetHtml", html, baseUrl, callback);
  } else {
    callback = baseUrl;
    this.command("SetHtml", html, callback);
  }
};

Browser.prototype.findText = function(text, callback) {
  this.command("FindText", text, callback);
};
...
Browser.prototype.command = function() {
  ...
  var _cbid;
  if (!callback._cbid) {
    _cbid = callback._cbid = this.callbacks.length;
    this.callbacks.push(callback);
  }
  commands.push(Buffer.byteLength(_cbid.toString()) + "\n" + _cbid);
