...
Browser.prototype.highlightRender = function(url, width, text, callback) {
  if (callback) {
    this.command("HighlightRender", url, width, text, callback);
  } else {
    callback = text;
    this.command("HighlightRender", url, width, '', callback);
  }
};
