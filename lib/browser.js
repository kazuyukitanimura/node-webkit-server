...
Browser.prototype.highlightRender = function(width, text, callback) {
  if (callback) {
    this.command("HighlightRender", width, text, callback);
  } else {
    callback = text;
    this.command("HighlightRender", width, '', callback);
  }
};
